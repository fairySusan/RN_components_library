...
export class PageListView extends Component{
    ...
    static defaultProps={
        ...
        refresh: null,
        isInitRefresh: true, // 是否自动初始化加载数据
        //父组件处理"加载更多"的方法
        loadMore:null,
        //每个分页的数据数
        pageLen:0,
    ...
    render() {
        ...
        return(
            !!this.refreshEnable?
                <View style={[{flex:1}, this.props.listStyle]} onLayout={this.onLayout}>
    ...
    scrollList=(y)=>{
        this.list&&(this.list.scrollToOffset({offset:y}));
    };
